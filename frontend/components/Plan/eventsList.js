const events = [
  {
    day: '23 de Novembro',
    title: 'Abertura das Inscrições',
    description:
      'As inscrições na nossa plataforma abrem e assim que te inscreveres terás acesso à carta da pessoa que farás feliz este Natal.',
  },
  {
    day: '24 de Novembro',
    title: 'Início de Recolha de Prendas',
    description:
      'Após comprar a prenda poderás entregá-la num conjunto de locais onde nós estaremos — visita o nosso Facebook para saberes o local onde poderás entregar a tua prenda.',
  },
  {
    day: '07 de Dezembro',
    title: 'Fim da Recolha de Prendas',
    description:
      'Tens até dia 7 para proceder à entrega da prenda. Após esta data, não poderemos aceitar mais nenhuma entrega.',
  },
  {
    day: '08 de Dezembro',
    title: 'Início das inscrições e entregas dos Rematches',
    description:
      'Infelizmente, nem toda a gente que se inscreve, cumpre a promessa de entregar. Para garantir que não há nenhum participante sem prenda, vamos abrir uma segunda fase de inscrições.',
  },
  {
    day: '16 de Dezembro',
    title: 'Fim dos Rematches',
    description:
      'A segunda fase de inscrições é mais curta e terás apenas uma semana para fazer a entrega da prenda. Os locais de entrega serão os mesmos que da primeira fase.',
  },
  {
    day: '17 de Dezembro',
    title: 'Início das Entregas',
    description:
      'Missão cumprida! Os Pais Natais vão para a estrada e durante 7 dias vão fazer entregas em várias instituições para garantir um Natal recheado de alegria e carinho.',
  },
];

export default events;
